...
const AddAchievement = async (req, res) => {
 try {
  ...
  if (!ValidateBody(['title', 'level', 'date','position','media'], req.body, res)) return

  const { title, level, date,position,media } = req.body

  const data = { title, level, date,position,media }
...
const EditAchievement = async (req, res) => {
 try {
  ...
  if (!ValidateBody(['id','title', 'level','position','media','date'], req.body, res)) return

  const {id,title,level,position,media,date} = req.body;
  ...
  for (let i = 0; i < user.achievements.length; i++) {
   ...
   if (old_achievement._id == id) {
    ...
    achievment.level = level
    achievment.position = position
    if (media && media.trim() !== "") {
    achievment.media = media;
    }
