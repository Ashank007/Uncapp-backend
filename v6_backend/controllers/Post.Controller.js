...
const GetAllUserPost = async (req, res) => {
 try {
  ...
  const data = { userAvatar: user.avatar, userName: user.username, post: user.posts }
 ...
 try {
  ...
  const followedUsers = await Promise.all(
   followingIds.map(async followedId => {
    const followedUser = await FindUserById(followedId, 'avatar fullname username posts', ['posts'])
...
const GetPost = async (req, res) => {
 try {
  ...
  const post = await Post.findById(id).populate("createdby","username avatar")
...
try {
  ...
  res.redirect(`myapp://post/${postId}`);
...
export { CreatePost, EditPost, DeletePost, GetAllUserPost, GetPost, LikePost, SavePost, GetAllSavePost, AddComment, GetComments, GetAllUserFollowingPost,SharePost}
